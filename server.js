...
app.use((req, res, next) => {
  if (req.path === '/env-config.js') {
    ...
    const envConfig = `
      window._env_ = {
        REACT_APP_API_URL: "${process.env.REACT_APP_API_URL || 'https://onnoto-backend-production.up.railway.app/api'}",
        ...
        REACT_APP_WS_URL: "${process.env.REACT_APP_WS_URL || 'wss://onnoto-backend-production.up.railway.app/ws'}"
      };
    `;
...
app.listen(port, () => {
  ...
  console.log(`API URL: ${process.env.REACT_APP_API_URL || 'https://onnoto-backend-production.up.railway.app/api'}`);
});